...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.systemPrompt.section({
    ...
    order: 115,
    text: resolved.write
      ? 'GitHub tools are available. Use github_search to find issues, PRs, repositories, or code; pass the owner/repo and number of a hit to github_issue_read or github_pr_read. PR reads are split into parts (metadata, diff, comments, checks) — request only what you need. Writes (github_issue_create, github_issue_comment, github_pr_create) require the user\'s approval; a denial is an answer, not an error. PR creation is idempotent: an already-open PR for the same branches is returned instead of failing.'
      : 'GitHub read tools are available. Use github_search to find issues, PRs, repositories, or code; pass the owner/repo and number of a hit to github_issue_read or github_pr_read. PR reads are split into parts (metadata, diff, comments, checks) — request only what you need.',
  })
